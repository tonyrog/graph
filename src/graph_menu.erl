...
draw(MenuState = #menu_state { row=Row}, Pixels, {X,Y}) ->
    ...
    epx_gc:set_border_color(
      graph_profile:color(Scheme,Profile#menu_profile.border_color)),
    epx_gc:set_fill_color(
      graph_profile:color(Scheme,Profile#menu_profile.background_color)),
    epx_gc:set_fill_style(solid),
    epx:draw_roundrect(Pixels,{X,Y,W,H},4,4), %% profile?
    MenuFontColor = graph_profile:color(Scheme,Profile#menu_profile.font_color),
